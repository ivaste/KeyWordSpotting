...
There are two types of Separable Convolution: Spatial\footnote{Spatial Separable Convolutions divide a kernel into two smaller kernels. With a lower number of multiplications for each convolution, the computational complexity significantly decreases, and the network is able to run and train faster.} (not considered here) and Depthwise.
